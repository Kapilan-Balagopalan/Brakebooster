    ...
    for i in tqdm(range(n_gap)):
        ...
        while (len(Arms) > 1):
            ...
            for j in Arms:
                ...
                # print("end")
                if (experiment_config == "SE"):
                    thresh = 2*np.sqrt(np.log(K*(t ** 2) / delta) / t)
                elif (experiment_config == "SE_LUCB_CB"):
                    thresh =  np.sqrt(np.log(1.25 * K * (np.sum(samples_count) ** 4) / delta) / (2*samples_count[j]) )
                else:
                    thresh = np.sqrt(2 * np.log(3.3 * (t ** 2) / delta) / t)

                #print(thresh)



                if (np.max(samples) - samples[j]  >= thresh):
...
def t():
    ...
    for i in range(n_cpu):
        pool.apply_async(SE, args=(n_gap,i))
    ...
    counts, bins, _ =  plt.hist(histogram_vector, bins=1000, color=color_list[esp_count], alpha=0.5, edgecolor=color_list[esp_count], lw=3, label= "SE")
